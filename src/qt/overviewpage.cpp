...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::XHM)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("XHM")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeXHimeraAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeXHimeraAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeXHimeraAmount = strAnonymizeXHimeraAmount.remove(strAnonymizeXHimeraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXHimeraAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeXHimeraAmount * COIN) nMaxToAnonymize = nAnonymizeXHimeraAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeXHimeraAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeXHimeraAmount));
        strAnonymizeXHimeraAmount = strAnonymizeXHimeraAmount.remove(strAnonymizeXHimeraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXHimeraAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
