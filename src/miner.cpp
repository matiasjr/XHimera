...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("XHimeraMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("XHimeraMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("XHimeraMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xhimera-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running XHimeraMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
